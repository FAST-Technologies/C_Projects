...
#define GENERATE_TEST(test_name, dest, src, n)       \
  START_TEST(test_name) {                            \
    ...
    s21_strncat(dest_custom, src, n);                \
    strncat(dest_original, src, n);                  \
    ck_assert_mem_eq(dest_custom, dest_original, n); \
  }                                                  \
  END_TEST

GENERATE_TEST(s21_strncat_test_1, "Hello, ", "World", 6)
GENERATE_TEST(s21_strncat_test_2, "12345", "678", 2)
GENERATE_TEST(s21_strncat_test_3, "qwert", "zxc", 7)
