...
#define GENERATE_TEST(test_name, string, expect) \
  START_TEST(test_name) {                        \
    char *result_custom = s21_to_upper(string);  \
    ck_assert_str_eq(result_custom, expect);     \
    if (result_custom) free(result_custom);      \
  }                                              \
  END_TEST

GENERATE_TEST(s21_to_upper_test_1, "good job", "GOOD JOB")
GENERATE_TEST(s21_to_upper_test_2, "GoOdByE", "GOODBYE")
GENERATE_TEST(s21_to_upper_test_3, "", "")
GENERATE_TEST(s21_to_upper_test_4, "1=-384673^", "1=-384673^")
GENERATE_TEST(s21_to_upper_test_5, "A675FF537fdsD", "A675FF537FDSD")
